...
public class ConfigEditor {
    ...
    public static void setupConfig() {
        try {
            ...
            for (Player current : Bukkit.getOnlinePlayers()) {
                SegmentManagerBG.initiateWarppouch(current.getName());
                SegmentManagerPS.initiatePlayerRecords(current.getName());
            }
    ...
    private static void initiateDebugLevel() {
        try {
            ...
            // Sets the debug level to the standard value of 1
            ConfigEditor.set("DebugLevel", 1);
            MessageMaster.sendSkipMessage("ConfigEditor", "initiateDebugLevel(), the DebugLevel path doesn't exist.");
        } catch (Exception e) {
    ...
    private static void initiateActivationstates() {
        try {
            ...
            Collections.addAll(paths, SegmentManagerCE.getActivationstatePaths());
            ...
            MessageMaster.sendSkipMessage("ConfigEditor", "Skipped method initiateActivationstates(), IllegalStateException");
        } catch (Exception e) {
     ...
     *
     * @param level [int] The debug level.
     */
    public static void setDebugLevel(int level) {
        try {
            ...
            ConfigEditor.set("DebugLevel", Main.getStandardDebugLevel());
        }
    }
    ...
    public static Set<String> getSectionKeys(String path) {
        try {
            ...
            if (keys.toArray().length > 0) {
                ...
                // No keys found
                MessageMaster.sendSkipMessage("ConfigEditor", "Skipped method getSectionKeys(" + path + "), no keys found");
            }

            return keys;
        } catch (NullPointerException e) {
            MessageMaster.sendSkipMessage("ConfigEditor", "Skipped method getSectionKeys(" + path + "), the path doesn't exist.");
            return null;
        } catch (Exception e) {
    ...
    public static boolean containsPath(String path) {
        try {
            ...
            MessageMaster.sendSkipMessage("ConfigEditor", "Skipped method containsPath(" + path + "), the path doesn't exist.");
            return false;
        } catch (Exception e) {
    ...
    public static boolean hasValue(String path, Object value) {
        try {
            ...
            boolean rightValue = get(path).equals(value);

            MessageMaster.sendSuccessMessage("ConfigEditor", "hasValue(" + path + ")");
            return rightValue;
        } catch (NullPointerException e) {
            MessageMaster.sendSkipMessage("ConfigEditor", "Skipped method hasValue(" + path + "), the path doesn't exist.");
            return false;
        } catch (Exception e) {
    ...
    /**
     * Saves the config file. Returns true if the process succeded.
     */
