...
interface SampleCode {
    ...
    private static boolean commandSamples() {
        ...
        xDATA_TYPEx xVARIABLEx = (xDATA_TYPEx) GlobalFunctions.getConfigObject("xCONFIG_PATHx", "xNULL_MESSAGEx", xSENDERx);
        if (xVARIABLEx == null) return true;

        // Gets or repairs config entry SC-1
        xDATA_TYPEx xVARIABLEx = xFIX_VALUEx;
        Object[] temp = GlobalFunctions.getConfigObjectFix("xCONFIG_PATHx", xVARIABLEx);
        if ((boolean) temp[0]) xVARIABLEx = (xDATA_TYPEx) temp[1];
        ...
        float xFLOAT_NAMEx = GlobalFunctions.introduceFloat(args[xINPUT_TO_INTRODUCEx], xMIN_VALUEx, xMAX_VALUEx, sender);
        if (xFLOAT_NAMEx == Float.MIN_VALUE) return false;
        if (xFLOAT_NAMEx == 0) return GlobalFunctions.invalidNumber(sender, args[xINPUT_TO_INTRODUCEx]);
