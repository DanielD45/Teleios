...
public class JoinmessageCommandListener implements CommandExecutor, Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        try {
            ...
            MessageMaster.sendSkipMessage("JoinmessageCommandListener", "Skipped method onPlayerJoin(" + event + "), the JoinMessage path in the config file doesn't exist.");
        } catch (Exception e) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        try {
            ...
            if (!ConfigEditor.isActive("AdminFeatures.All")) {
                ...
                MessageMaster.sendSkipMessage("JoinmessageCommandListener", "Skipped method onCommand(" + sender + ", " + command + ", " + label + ", " + Arrays.toString(args) + "), the command is deactivated.");
                return true;
            }
            ...
            if (!sender.hasPermission("teleios.adminfeatures.joinmessage")) {
                ...
                MessageMaster.sendSkipMessage("JoinmessageCommandListener", "Skipped method onCommand(" + sender + ", " + command + ", " + label + ", " + Arrays.toString(args) + "), the sender doesn't have the needed permissions.");
                return true;
            }
            ...
            if (args[0].equalsIgnoreCase("enable") || args[0].equalsIgnoreCase("true")) {
                ...
                sender.sendMessage("§aThe join message is now §6enabled§a!");
                ...
                sender.sendMessage("§aThe join message is now §6disabled§a!");
                MessageMaster.sendSuccessMessage("JoinmessageCommandListener", "onCommand(" + sender + ", " + command + ", " + label + ", " + Arrays.toString(args) + ")");
                return true;
            }
            else {
                sender.sendMessage("§cWrong arguments!");
                MessageMaster.sendSkipMessage("JoinmessageCommandListener", "Skipped method onCommand(" + sender + ", " + command + ", " + label + ", " + Arrays.toString(args) + "), wrong arguments.");
                return false;
            }
